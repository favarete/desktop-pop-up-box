...
loop
{
   ...
   loop % count
   {
      ...
      loop % count
      {
         ...
         Loop {
            Process, Close, % "desktopopup.exe"
            Process, Exist, % "desktopopup.exe" 
         ...
         stringAfter := text
         Loop, Parse, stopWords, `n, `r
            stringAfter := RegExReplace( stringAfter, "i)\b" A_LoopField "\b")

         stringAfter := Trim(stringAfter)
         Run "S:\Executables\Desktopopup\desktopopup.exe" -- "%stringAfter%"
         Send {F11}
      }
